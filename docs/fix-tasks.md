...
- **Новый найденный дефект, не входящий ни в одну задачу T00–T14, вне SCOPE каждой из них.** `slug()` в `run.sh`/`review.sh` строит имя как `${task#$ROOT/}`, где `$task` приходит из `tasks.py` (`pathlib.Path(args.root).resolve()` — резолвит символические ссылки) и должен начинаться с `$ROOT` (bash: `$(cd "$PROJECT" && pwd)` — **не** резолвит символические ссылки). Если путь к проекту проходит через символическую ссылку (на macOS это всегда так для `/tmp` и `/var`; на Linux — при симлинке `/home` или смонтированном каталоге), `$ROOT` перестаёт быть буквальным префиксом `$task`, `#`-срез не отрезает ничего, и `slug()` превращает в имя файла **весь абсолютный путь**, а не относительный путь задачи. Воспроизведено: `ROOT=/var/folders/…/tasks`, `task=/private/var/folders/…/tasks/10-a/01-x` — не совпадает префиксом. Само по себе это не ломает работу харнесса (`run.sh` и `review.sh` вычисляют одинаково «неправильное» имя независимо, поэтому переиспользование между ними не страдает), но имена файлов в `.hermes-harness/logs/` становятся нечитаемыми и склонны упираться в предел длины имени файла (около 255 байт) на глубоких путях — особенно после T12, который добавляет к имени метку времени и номер попытки. Нужна отдельная задача: либо резолвить `$ROOT` в bash так же, как в Python (`cd -P` вместо `cd`), либо сравнивать пути в `slug()` не строковым префиксом. Для T05 это было обойдено на уровне теста (см. `L=…` в T05, комментарий в `selftest.sh`), не в коде.
