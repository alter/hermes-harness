...
while :; do
  ...
  outcome=$(python3 - "$LOGS/$name.review.json" "$task" "$rc" "$TEST_CMD" <<'PY'
...
# project's own check, which nothing else can stand in for.
denied_commands = [str(d.get("tool_input", {}).get("command", d.get("tool_name", "?"))) for d in denials]
if test_cmd:
    head = test_cmd.split()[0]
    if any(head in c for c in denied_commands):
...
PY
  ...
  if [ "$usable" != "yes" ]; then
    ...
    if [ "$rounds" -ge "$MAX_ROUNDS" ]; then
      ...
      echo "   the review could not be relied on; see $task/REVIEW.md and $LOGS/$name.review.err"
    fi
    continue
  fi
